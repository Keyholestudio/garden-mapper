"""Regen Onion sticker — no soil, chroma-key green bg"""
...
RAW_DIR    = os.path.join(WORKSPACE, "stickers", "generated", "rootveg")
CLEAN_DIR  = os.path.join(WORKSPACE, "stickers", "generated", "rootveg", "processed")
DEST       = os.path.join(WORKSPACE, "app", "public", "stickers")

PLANT_ID   = "vegetable-root_onion_S_CA-US-FR-GB-AU"
...
# Root veg template + "no soil" added
PROMPT = (
    ...
    "Subject: Onion, bulb vegetable.\n"
    "Canvas: 160px square.\n"
    "Colours: papery golden-brown #C8A050, pale white #F5F0E8, "
    "bright green strap tops #4A8C3A, mid-green #2A6010, dark outline #1A0A0A, "
    "flat chroma-key green background (#00FF00)\n"
    "Shape: Onion bulb and strap tops only. No soil, no ground, no dirt, no roots. "
    "Round papery golden-brown onion bulb floating cleanly, strap-leaf tops above. "
    "Natural proportions."
)
...
if not found:
    print("FAIL: no image after 240s"); exit(1)
...
if data_url.startswith("data:image"):
    ...
    print(f"RAW saved (canvas): {len(img_bytes)//1024}KB")
...
if result.returncode != 0:
    print(f"Pipeline error: {result.stderr}"); exit(1)
