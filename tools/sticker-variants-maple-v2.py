"""
Garden Mapper — Maple Variant Regeneration (Dark Green + Silver-Green)
No trunk. Correct style. Saves to stickers/generated/pending/
"""
...
VARIANTS = [
    {
        ...
        "prompt": (
            f"{STYLE}\n\n"
            "Subject: Maple Tree canopy, large broad deciduous tree. NO TRUNK. Canopy only.\n"
            "Canvas: 512px square.\n"
            "Colours: Deep dark green foliage (Norway Maple / Crimson King spring/summer) — "
            "dark green #2E7D32, very dark #1B5E20, mid-green #388E3C. "
            "flat chroma-key green background (#00FF00)\n"
            "Shape: Natural full leafy canopy, distinctive lobed maple leaf shapes visible. "
            "No trunk. No branches. Canopy fills the frame. Spring/summer only. No fall colours."
        )
    },
    {
        ...
        "prompt": (
            f"{STYLE}\n\n"
            "Subject: Silver Maple Tree canopy, large broad deciduous tree. NO TRUNK. Canopy only.\n"
            "Canvas: 512px square.\n"
            "Colours: Silver Maple — green-grey top with silvery-white leaf undersides visible, "
            "grey-green #78909C, silver-white highlights #ECEFF1, medium grey-green #607D8B. "
            "flat chroma-key green background (#00FF00)\n"
            "Shape: Natural full leafy canopy, lobed maple leaf shapes with silvery sheen. "
            "No trunk. No branches. Canopy fills the frame. Spring/summer only. No fall colours."
        )
    },
]
...
def generate_one(variant):
    ...
    # Remove existing so it regenerates
    ...
    p(f"  Prompt sent: {sent} | Waiting up to 240s...")
    ...
    if not found:
        p(f"  FAIL: no image after 240s"); return False
    ...
    if not data_url:
        p(f"  FAIL: could not get image data"); return False
    ...
    if data_url.startswith("data:image"):
        ...
        p(f"  FAIL: unexpected data format"); return False
    ...
    if os.path.exists(tmp_nobg):
        ...
        p(f"  DONE -> {filename}")
        return True
    p(f"  FAIL: no pipeline output"); return False

p("Maple Variant Regeneration — Dark Green + Silver-Green")
p("Brave must be open with Gemini loaded.\n")
