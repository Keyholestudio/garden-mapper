"""Regen Oak Tree v2 — Rob's exact prompt"""
...
# Rob's exact prompt — do not modify
PROMPT = """Side aerial view. Art style: Plants vs. Zombies meets watercolor painting — tasteful simplified representation of this plant with crisp edges, focusing on a primary characteristics of the plant, leafy, Dark outline 2-3px. No shadows. Centered, 75% canvas fill. Vibrant and iconic.

Subject: Oak Tree, large broad deciduous tree, No trunk.
Canvas: 512px square.
Colours: oak green, mid-green #4A8C3A, olive green #6B7A3A, dark outline #0A1A0A, flat chroma-key green background (#00FF00)
Shape: Natural broad leafy canopy. NO border. NO frame. NO white or grey edges."""
...
# Pipeline with raised tolerance for clean edge removal
import tempfile
with open(PIPELINE, "r") as f:
    src = f.read()
patched = src.replace("TOLERANCE  = 80", "TOLERANCE  = 120").replace("SOFT_RANGE = 40", "SOFT_RANGE = 60")
patched_path = os.path.join(tempfile.gettempdir(), "pipeline_oak.py")
with open(patched_path, "w") as f:
    f.write(patched)

print("Running pipeline (tolerance=120)...")
