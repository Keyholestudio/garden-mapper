...
- `flower-daisy_echinacea_M_CA-US-FR-GB.png` — old file, no AU region, what the catalog pointed to
- `flower-daisy_echinacea_M_CA-US-FR-GB-AU.png` — new file, generated by sticker-generate-one.py

Every regen updated the `CA-US-FR-GB-AU` file. The catalog kept loading the old `CA-US-FR-GB` file. No error, no warning — both files existed so the validator passed clean.

**Root cause:** Early stickers were generated with shorter region codes (no AU). Later the generator standardised on `CA-US-FR-GB-AU`. When regenerating, the old file was never deleted and the catalog `src` was never updated.

**How to catch it before it bites:**
1. **When regenerating any sticker (`--force`):** check for any other PNG in `app/public/stickers/` that shares the same key prefix but a different region suffix. Delete the old one.
2. **After any regen:** confirm `usePlantCatalog.js` `src` matches the filename the generator produced. If they differ — update the catalog.
3. **The validator doesn’t catch this** — it only checks that a referenced file exists, not that it’s the right/current one.

**Quick check command:**
```powershell
# Find any sticker key with multiple region-code variants
...
```
Run this after any batch regen session. If a key has more than 1 file — investigate and remove the old one.
